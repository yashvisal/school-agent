...
**Two navigation modes** (decided 2026-09-01 after the first M1 walkthrough — the sidebar is the primary navigation primitive):

- **Global mode** — sidebar: **Dashboard · Semester · Connectors · Settings · — · courses.** The student's whole semester at a glance. No global Library.
- **Course mode** — clicking a course *enters* it: the sidebar becomes course-scoped — its **header row becomes the course** (chevron-left in the workspace glyph's slot, course code in the product name's slot; the row is the way back to global mode), then **Overview** · **Library** · **Chats** ("New chat" on the section header). Each chat entry is meant to be a *durable* eve session that reopens where it was left; until Core stores chats, the entries are fixture-backed and the transcript behind one does not persist. The course is a place you go, not a filtered page. Opening a chat puts the conversation in the **viewport** (the harness's native shape: sidebar of chats, chat as the main pane); the rail carries **Context** and **Tasks**. Library is per-course, Drive-like: uploads, Canvas captures, and later the agent's artifacts and lessons.
