...
- **Course mode** — clicking a course *enters* it: the sidebar becomes course-scoped — its **header row becomes the course** (chevron-left in the workspace glyph's slot, course code in the product name's slot; the row is the way back to global mode), then **Overview** · **Library** · **Chats** (list of persisted sessions, "New chat" on the section header). The course is a place you go, not a filtered page. Opening a chat puts the conversation in the **viewport** (the harness's native shape: sidebar of chats, chat as the main pane); the rail carries **Context** and **Tasks**. Library is per-course, Drive-like: uploads, Canvas captures, and later the agent's artifacts and lessons.
