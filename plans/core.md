...
- `students` — clerkId, timezone, phone, semester start/end, **class schedule** (hard blocks: parsed from an uploaded schedule image/file or Canvas sections, approved by the student), availability (weekly template + exceptions), morning hour, `checkInPreference` (`fewer | normal | more` — a stated preference the agent reads, never a computed cadence). The student edits all of it through `api.students.updatePrefs`, which goes through `changes` with origin `manual` and approves in the same call (Approval channels rule 1). Two routing-bookkeeping fields sit outside `changes` because they are facts about our transport, not about the student: `inboundCount` and `photonRegistration` (`{ status: registered | failed | skipped, at, error? }`) — **saving a phone schedules its registration with Photon**, since a shared line can only message a registered user (voice.md "Deliverability"; VOICE_TOOLS.md §8b).
...
- `materials` — courseId, file ref (Convex storage), origin, `folderPath` (M3 Library placement; backfill default "From Canvas", or the Canvas module name when present — workspace.md "Filing"). Captured now for Milestone 3. Materials are upserted directly, **not** through `changes`: they are raw source captures (files/modules/pages/announcements as Canvas served them), not facts about the student's obligations — nothing to approve, nothing the planner reads. The `changes` rule covers student state.
...
- `auto`: changes from authoritative structured sources (Canvas, iCal) apply immediately and appear in the change feed as "new since you last looked." Two non-source origins are also `auto` because they claim nothing about the world that Core has not itself verified: `planner` (Slice 1 — the agent's picks from Core's own feasible set, each verified against `fits` before applying) and, from M3, `workspace` (an artifact for a `prepared` task exists — an observation, not an interpretation). Neither can be asserted through the generic `proposeChange` route, which forces origin `chat`.
...
- `planner` (decided 2026-09-05): the agent committing the day it just texted applies immediately, like a structured source and unlike anything else it can write. The tier keys off what a change *claims about the world*, and a plan commit claims nothing — it is the agent choosing among options Core itself computed, which is the plan rather than an interpretation of something the student said, and it carries no provenance because it asserts no fact. What makes it safe is that Core re-verifies every pick against its own feasible set (same option, inside a free window, ending by the due minute) before applying, and refuses the whole commit otherwise. Only `commitPlan` emits this origin; `proposeChange` still forces `chat`, so Voice cannot reach the `auto` tier by asking for it.
...
**Nightly precompute (Convex cron):** the name is historical — the pass is a *morning* pass, and the module, cron, and `operationId` prefix keep the old word only because they are already deployed. It fires hourly and picks out each active student whose local clock just struck their **morning hour** (`morningHourLocal`, default 7am), then computes **that same day's** feasible set, pending-change annotations, and a signals digest, stores the snapshot, and triggers the eve Voice run (an idempotent `operationId` per student-day). Today, not tomorrow: a 4am run describing the day after next is a text about a day the student cannot act on, and the planner already subtracts the hours already elapsed, so a 7am plan offers only the part of the day still ahead. A failed or stuck trigger is retried by later hourly passes for up to 6 hours after the student's morning hour; missing eve config records `skipped` rather than failing the pass. Convex decides who gets a run (paginated, one scheduled run per student); eve decides what to say (voice.md M1 #2).
...
- **The four Voice tools** (the entire surface Voice has on Core): `getFeasibleActions(studentId, date)` — options annotated with facts, effort priors, relevant `studentSignals`, and `planned` (the block already committed for that day); `proposeChange(change)` — the mutation path for anything the agent *learned*, always through `changes` with tier/approval semantics, origin forced to `chat` server-side (Voice cannot claim an authoritative origin or supply provenance, so it can never self-elevate to the `auto` tier; on `students`, chat-origin changes may touch only the five scheduling fields — never phone/status/timezone/identity); `commitPlan(date, picks)` — the plan itself, 1–3 blocks, each re-verified against the same feasible set the agent was shown before it applies at origin `planner`, and authoritative for that date (agent tasks it drops come back unplanned, never skipped; student tasks are untouched); `recordSignal(signal)` — writes `studentSignals`. Both write paths go through `changes`, and nothing else planning-shaped is reachable from the agent; two bookkeeping routes ride alongside (`logUsage` — the mandatory per-call cost log, vision §10 — and `resolveStudent`, phone → student, needed before any tool can be called). Contract: `convex/VOICE_TOOLS.md`.
