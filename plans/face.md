...
**Two navigation modes (decided 2026-09-01; vision §8).** *Global mode*: sidebar Dashboard · Semester · Connectors · Settings · courses; viewport + Context/Tasks rail. *Course mode*: clicking a course swaps the sidebar contents — the header row itself becomes the course identity *and* the way back out (a chevron-left in the workspace glyph's slot, the course code in the product name's slot, the whole row links to `/`; refined 2026-09-01, replacing the earlier separate `← All courses` row), then Overview · Library · Chats; Overview is the landing page; Chats is a per-course list with "New chat" on the section header; opening one puts the conversation in the **viewport** (harness-native), rail = Context + Tasks, no Chat tab in the rail. **Target state**: each entry is a *persisted* eve session that reopens where it was left. **Today**: the entries are fixture-backed and the transcript is not persistent — a chat id selects a fresh eve session, so reopening one shows an empty pane. Durable chats need a Core `chats` table plus session restoration (`initialSession` + `resume`); TODO(core) in `components/workspace/chat-transcript.tsx`. Library is per-course only (no global Library). The first M1 walkthrough built the course as a filtered dashboard with chat in the rail — that was a doc gap, not a thread error; this section is the correction. Designs for both modes live in Paper (see "Design flow") and are reviewed there before building.
...
2. **Streaming — DONE, live end-to-end.** An eve session streams `dynamic-tool` parts into the forked harness and renders an approval card and a diff table. The glue is a pure reducer, `lib/eve/reduce.ts` (`EveMessage[] → RailItem[]`: `reasoning` → thinking, `text` → streaming prose, `dynamic-tool` → tool chips by `toolName`, a gated `propose_change` in `approval-requested` → approval card + diff table wired to `respond([{ requestId, optionId }])`, `output-available` → a collapsed "change proposed — pending in Core" row); `components/workspace/chat-transcript.tsx` renders it under `useEveAgent({ agent: "workspace" })` — in the **viewport** since the nav refactor, at `/courses/[courseId]/chats/[chatId]`; it was the rail when this was written. **Verified live in the browser** on `/courses/course_algos` with `anthropic/claude-haiku-4.5`: prompt → tool call → approval card with the before/after diff → Approve → `output-available` row, signed in through Clerk. The route auth that admitted that traffic was `localDev()`, which is exactly what it is for — `agent/workspace/channels/eve.ts` runs `[vercelOidc(), localDev()]`, and **in a deployment neither verifier admits a browser**, so deployed browser streaming 401s until a Clerk verifier is added (TODO(face) in that file). Deliberate: fail-closed beats a permissive channel while the seam is still moving, and Spike B's question was whether the parts render, not who may send them. Also verified without a model by replaying `lib/eve/fixtures.ts` frames through the reducer — 36/36 in `scripts/spike-b-stream-check.mts`, and a dev-only `?replay=1` mode feeds the same fixture through the same renderers so the card is demoable with the gateway down. Whole run cost well under a cent.
