...
/* Upstream fires a `glimm` rainbow shader sweep across the composer when the
 * flagship model is picked. `glimm` is stripped from this fork (no new deps),
 * so model selection is a plain state change. */
...
export default function PromptBar({
  ...
  /* reset the highlighted row when the menu or its query changes */
  const menuKey = `${menu}:${query}`;
  const [activeFor, setActiveFor] = useState(menuKey);
  if (activeFor !== menuKey) {
    setActiveFor(menuKey);
    setActive(0);
    setEngaged(false);
  }
  ...
  const [hoverResetFor, setHoverResetFor] = useState(modelOpen);
  if (hoverResetFor !== modelOpen) {
    setHoverResetFor(modelOpen);
  ...
  useEffect(() => {
    ...
    /* applied on the next frame rather than in the effect body, so the demo
     * loop never cascades renders */
